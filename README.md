...
Для длительного разбора добавьте `--progress`. Анализатор будет выводить в
`stderr` текущую фазу, обработанный объём, скорость, прошедшее время и
приблизительный общий процент/ETA; итоговый JSON в `stdout` и сохранённые
артефакты останутся без изменений. Первые несколько секунд ETA отображается как
`calculating`, пока не накоплена оценка скорости.

```powershell
& $TjPython -B "$TjToolDir\analyze_1c_tj.py" 'C:\Path\To\1clogs' --output-dir $TjAnalysis --progress
```

Для машинного чтения используйте `--progress-format jsonl`. Частоту обновлений
можно ограничить параметром `--progress-interval`; значение задаётся в секундах
и должно быть больше нуля. Общий процент является фазовой оценкой: фактическое
время связывания, агрегации и экспорта зависит от состава событий, а не только
от размера исходных файлов.
